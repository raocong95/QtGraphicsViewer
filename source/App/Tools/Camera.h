...
public:
	...
	Camera() : mViewIsUptodate(false), mProjIsUptodate(false), mProjectionIsPerspective(true), 
		start(Vector3f::Zero()), delta(Vector3f::Zero())
    {
		...
		// default settings
		setPosition(Vector3f(0, 2, 10));
		setTarget(Vector3f(0,0,0));  // change orientation to face target
		setPosition(position() + 2 * up());  // translate camera without changing the target
    }
    ...
    void zoom(float d)
    {
		Vector3f target2me = position() - mTarget;
		Vector3f target2meNew = (1 + d) * target2me;

		double dist = target2meNew.norm();
		if (dist > 0.1 && dist < 100)
		{
			setPosition(mTarget + target2meNew);
			mViewIsUptodate = false;
		}
...
protected:
    ...
    void updateProjectionMatrix(void) const
    {
        if(!mProjIsUptodate)
        {
			...
			{
				QMatrix4x4 projM;
				projM.ortho(0, 1, 0, 1, mNearDist, mFarDist);
				mProjectionMatrix = Eigen::Matrix4f(projM.transposed().data());
			}
