                    ...
                    const bool has_colors)
{
	...
	typedef Vec3 Color;
	typedef Eigen::Matrix<int, 3, 1> ColorI;
	...
	Surface_mesh::Vertex_property<ColorI>              colors_integer;
	...
	if (has_colors)    colors_integer = mesh.vertex_property<ColorI>("v:colori");
    ...
    for (i=0; i<nV && !feof(in); ++i)
    {
        ...
        // position
        //items = sscanf(lp, "%f %f %f%n", p.x(), &(p[1]), &(p[2]), &nc);
		items = sscanf(lp, "%f %f %f%n", &px, &py, &pz, &nc);

        assert(items==3);
        //v = mesh.add_vertex((Vec3)p);
		v = mesh.add_vertex(Vec3(px, py, pz));
        lp += nc;
        ...
        if (has_colors)
        {
            if (sscanf(lp, "%lf %lf %lf%n", &c[0], &c[1], &c[2], &nc) == 3)
            {
				if (c[0] > 1.0f || c[1] > 1.0f || c[2] > 1.0f) {
					colors_integer[v] = (c + Vec3(0.00000001f, 0.00000001f, 0.00000001f)).cast<int>();
				}
				else {
					auto c_tmp = c * 255;;
					colors_integer[v] = c.cast<int>();
				}
